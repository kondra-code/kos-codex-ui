...
Use `kosui model` to set up a model, give it a name (in this case `service-example`) then set up a container model, and then say `yes` when prompted to set up services.
...
We can then create a hook with `kosui hook` and a component with `kosui component` and within the component, use the hook iterate over the list as normal, and render the name of each service-example.
