...
`kosui hook -i` generates hooks and higher-order components that can access models to provide the UI with data. Both have trade-offs - the hook is simple to use but the model is not guaranteed so some handling needs to take place to provide type safety.
...
KOS provides a generator to create model contexts, which provides the tools needed to access the model without multiple HoCs or hooks. Use `kosui context -i` to generate the appropriate files, then use the `provider` higher up in the tree and use the hook provided to access it further down.
