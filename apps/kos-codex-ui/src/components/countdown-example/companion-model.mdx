...
To create a companion model we use `kosui model:companion -i` which will set up the proper typing. We also need to register the model, as well as _register it as a companion model_ within the registration chain.
