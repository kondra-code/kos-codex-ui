...
import { MultiFuturesStory } from "./multiple-futures.stories";
...
It can also be set up as future-aware by adding the `--futureAware` flag when creating the model with the CLI.

```
kosui model --name futures --container --futureAware
```
...
A model can also handle multiple futures at once. This is useful when you have multiple operations that need to be tracked independently.

This is done by using the `@kosMultipleFutureAware` decorator on the model, and adding the `alias` option to the `@kosFuture` decorator to your methods. This will create a separate future for each alias, accessible via `${alias}Future`.


<Snippet name="multi-future-model@multi-futures-model.ts" />

<Canvas of={MultiFuturesStory} />
